...
class PostHazard:
    def __init__(self, effectiveDate=None, filename=None, verbose=False):
        ...
        inputFilename = filename if filename is not None else 'data/PostHazardRates.csv'

        if verbose:
            print(f'Loading PostHazard data from {inputFilename} for {self.effectiveDate}')
...
if __name__ == '__main__':
    ...
    postHazard = PostHazard(effectiveDate=effectiveDate)

    print('PostHazard data:')
    print(postHazard.data)